...
export const CompetenciesSetting = ({
    ...
    const [selectedCompetenceIds, setSelectedCompetenceIds] = useState<number[]>(initialCompetenceIds);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const groupsResponse = await fetch('http://host.docker.internal:8000/competency-groups/');
                if (!groupsResponse.ok) throw new Error('Ошибка загрузки групп компетенций');
                const groupsData = await groupsResponse.json();
                setCompetenceGroups(groupsData);

                // Загрузка всех компетенций
                const compResponse = await fetch('http://host.docker.internal:8000/competencies/');
    ...
    useEffect(() => {
        ...
        const fetchGroupName = async () => {
            try {
                const response = await fetch(`http://host.docker.internal:8000/competency-groups/${currentCompetence.competency_group_id}/`);
    ...
    const filteredCompetencies = allCompetencies.filter(comp => {
        const matchesGroup = selectedGroup ? comp.competency_group_id === selectedGroup : true;
        const matchesSearch = comp.code.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleCompetenceClick = (competence: Competence, event: React.MouseEvent) => {
    ...
    const groupedSelectedCompetencies = competenceGroups.map(group => ({
        group,
        competencies: selectedCompetences.filter(c => c.competency_group_id === group.id)
            ...
            <div className={styles.modal}>
                <div className={styles.modalHeader}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        <div
                            className={`${styles.loadingSpinner} ${isLoading ? styles.active : ""}`}
                        />
                ...
                <div className={styles.contentContainer}>
                    ...
                    <div className={styles.competenciesColumn}>
                        ...
                        <div className={styles.itemsList}>
                            {filteredCompetencies.map(comp => (
                                <div
                                    key={comp.id}
                                    className={`${styles.competenceItem} ${isSelected(comp.id) ? styles.selected : ""}`}
                    ...
                    <div className={styles.descriptionColumn}>
                        <h4 className={styles.descriptionTitle}>Информация о компетенции</h4>
                        <div className={styles.descriptionContent}>
                            {currentCompetence ? (
                                <>
                                    <p><strong>Код компетенции:</strong> {currentCompetence.code}</p>
                                    <p><strong>Группа компетенций:</strong> {currentGroupName}</p>
                                    <p><strong>Описание:</strong> {currentCompetence.name}</p>
                                </>
                            ) : (
                                <p className={styles.emptyState}>Выберите компетенцию для просмотра информации</p>
                            )}
                        </div>
