...
export const ReferenceForm = ({onClose}: { onClose: () => void }) => {
    const [selectedReference, setSelectedReference] = useState<ReferenceConfig | null>(null);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const response = await fetch(`http://host.docker.internal:8000${selectedReference.path}/`);
                if (!response.ok) throw new Error(`Ошибка загрузки: ${response.statusText}`);
                const data = await response.json();
                ...
                if (data.length > 0) {
                    ...
                    const itemResponse = await fetch(
                        `http://host.docker.internal:8000${selectedReference.path}/${data[0].id}/`
                    );
                    if (!itemResponse.ok) throw new Error(`Ошибка загрузки элемента: ${itemResponse.statusText}`);
    ...
    const handleItemClick = async (item: ReferenceItem | "add") => {
        if (item === "add") {
            ...
            const newItemData = selectedReference?.fields.reduce((acc, field) => {
                acc[field.key] = '';
                return acc;
            }, {} as Record<string, any>) || {};
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const url = selectedItem
                ...
                : `http://host.docker.internal:8000${selectedReference.path}/`;
            ...
            if (selectedItem) {
                setItems(items.map(item => item.id === selectedItem.id ? savedItem : item));
            } else {
                // После создания нового элемента обновляем список
                const listResponse = await fetch(`http://host.docker.internal:8000${selectedReference.path}/`);
                if (listResponse.ok) {
        ...
        try {
            ...
            if (listResponse.ok) {
                ...
                if (updatedList.length > 0) {
                    ...
                    const itemResponse = await fetch(
                        `http://host.docker.internal:8000${selectedReference.path}/${updatedList[0].id}/`
                    );
                    if (itemResponse.ok) {
                        const itemData = await itemResponse.json();
                        setSelectedItem(itemData);
            ...
            <div className={styles["reference-form"]}>
                <div className={styles["form-header"]}>
                    ...
                    <div style={{display: 'flex', alignItems: 'center'}}>
                        <div
                            className={`${styles["loading-spinner"]} ${isLoading ? styles["active"] : ""}`}
                        />
                    ...
                    <div className={styles["form-content"]}>
                        <div className={styles["items-container"]}>
                            ...
                            <div className={styles["items-list"]}>
                                <div
                                    className={`${styles["item"]} ${styles["add-item"]} ${!selectedItem ? styles["selected"] : ""}`}
